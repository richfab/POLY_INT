...
<select id="period_id" class="select-dropdown last">
    ...
    <option value="1" date-min="<?= date_sub(date_create('now'),date_interval_create_from_date_string('3 years'))->format('Y-m-d');?>" date-max="<?= date('Y-m-d');?>" >< 3 ans</option>
    <option value="2" date-min="<?= date_sub(date_create('now'),date_interval_create_from_date_string('1 year'))->format('Y-m-d');?>" date-max="<?= date('Y-m-d');?>" >< 1 an</option>
