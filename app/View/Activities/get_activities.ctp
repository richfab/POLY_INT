...
<?php foreach($activities as $activity):?>
    ...
    <?php if($activity['Activity']['type'] == 'experience'): ?>
            ...
            <div class="activity-subject">
                <?= $this->Html->link($activity['User']['firstname'].' '.$activity['User']['lastname'],array('controller'=>'users', 'action' => 'profile', $activity['User']['id']));?> <?= __("a ajouté une expérience");?>
            </div>
    ...
    <?php if($activity['Activity']['type'] == 'photo'): ?>
            ...
            <div class="activity-subject">
                <p><?= $this->Html->link($activity['Experience']['User']['firstname'].' '.$activity['Experience']['User']['lastname'],array('controller'=>'users', 'action' => 'profile', $activity['Experience']['User']['id']));?> <?= __("a ajouté des photos à");?> <?= $activity['Experience']['City']['name']; ?>, <?= $countries[$activity['Experience']['City']['country_id']];?></p>
            </div>
            <div class="activity-content">
    ...
    <?php if($activity['Activity']['type'] == 'recommendation'): ?>
            ...
            <div class="activity-subject">
                <?= $this->Html->link($activity['Experience']['User']['firstname'].' '.$activity['Experience']['User']['lastname'],array('controller'=>'users', 'action' => 'profile', $activity['Experience']['User']['id']));?> <?= __("a ajouté un bon plan à");?> <?= $activity['Experience']['City']['name']; ?>, <?= $countries[$activity['Experience']['City']['country_id']];?>
            </div>
            <div class="activity-content">
                <ul class="icons-list">
                    <li>
    ...
    <?php if($activity['Activity']['type'] == 'user'): ?>
            ...
            <div class="activity-subject">
                <?= $this->Html->link($activity['User']['firstname'].' '.$activity['User']['lastname'],array('controller'=>'users', 'action' => 'profile', $activity['User']['id']));?> <?= __("a rejoint la communauté Polytech Abroad");?>
            </div>
    <?php endif;?>
            ...
            <div class="activity-actions">
                ...
                <?php if(in_array($activity['Activity']['type'], array('photo','recommendation','experience')) && count($activity['Activity']['people_around']) > 1):?>
                    <a class="pull-left people-around hidden-xs" data-toggle="tooltip" title="<?php foreach ($activity['Activity']['people_around'] as $user) { echo $user['firstname'].' '.$user['lastname'].'<br/>';} ?>"><i class="fa fa-users"></i> <?php echo count($activity['Activity']['people_around']); ?> <?= __("personnes dans cette ville");?></a> 
                <?php endif; ?>
