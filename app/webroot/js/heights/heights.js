...
d3.json("heights/get_heights.json", function(error, data) {
    ...
    function init(){
        ...
        var bars = svg.selectAll("rect")
                ...
                .enter()
                .append("rect")
                .on("mouseover", changeWebsiteFromBar)
...
function changeWebsiteFromBar(){
    changeWebsite($(this).attr("index"));
}

// this function will be run everytime we mouse over an element
    function changeWebsite(school_id) {
        ...
        //on selectionne la bar correspondante
        var bar = d3.select(".bar.animationComplete[index='"+school_id+"']");
        ...
        //on selectionne le site correspondant
        var websiteLiName = $(".websiteLi[index='"+school_id+"'] span");
        websiteLiName.addClass("selected");

        //on change les données
        var websiteLi = $(".websiteLi[index='"+school_id+"']");
